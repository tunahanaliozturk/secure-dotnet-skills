...
- **Cookie auth: `HttpOnly`, `Secure`, `SameSite`, and sign-out.** In `AddCookie(opts => opts.Cookie.HttpOnly = true; opts.Cookie.SecurePolicy = CookieSecurePolicy.Always; opts.Cookie.SameSite = SameSiteMode.Lax)`. `SameSite = None` without `Secure = true` is rejected by modern browsers and exposes the cookie to cross-site requests. On sign-out, call `HttpContext.SignOutAsync(CookieAuthenticationDefaults.AuthenticationScheme)` and, for OIDC federated sessions, `HttpContext.SignOutAsync(OpenIdConnectDefaults.AuthenticationScheme)` to trigger the end-session endpoint.
...
| `SameSite = SameSiteMode.None` without `Secure = true` | The cookie is sent on cross-site requests (CSRF vector) and the `None` attribute is rejected by browsers if `Secure` is not also set, breaking authentication entirely in secure contexts. |
